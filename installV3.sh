...
function install_tunnel() {
    ...
    if [[ $tunnel_type == "6to4" ]]; then
        ...
        if [[ $server_type == "iran" ]]; then
            ...
            read -p "Enter the Relay server IP (this Server) address (e.g. 1.1.1.1): " RIP
            read -p "Enter the main server IP address (e.g. 2.2.2.2): " IP
            read -p "Enter the excluded ports (comma-separated, e.g. 22,51): " PORTS
            ...
            commands+=("echo -e '\033[92mThe tunnel was established for all ports except $PORTS\033[0m'")
            commands+=("read -p 'Back to Main menu? (y/n): ' answer; if [ \"\$answer\" == \"y\" ]; then sudo dds-tunnel; else echo 'OK'; echo -e '\033[36mExiting...\033[0m'; exit 0; fi")
        else
            commands=(
                "ip tunnel add ip6mhm mode ipip6 remote $foreign_ipv6 local $iran_ipv"
